...
func RunServerMigrations(config *config.Config, logger *logger.Logger) error {
	return RunPG(config.DatabaseDSN, embedMigrations, DirPostgres, logger)
}

// RunClientMigrations applies all SQLite migrations embedded in the binary.
// This is intended for client-side migrations.
func RunClientMigrations(config *config.Config, logger *logger.Logger) error {
	return RunSQLite(config.DatabaseDSN, embedMigrations, DirSQLite, logger)
}
